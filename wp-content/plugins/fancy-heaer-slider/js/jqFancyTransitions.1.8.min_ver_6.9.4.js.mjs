...
$('.ft-'+el.id).attr('href',links[el.id][imgInc[el.id]]);if(opts[el.id].position=='curtain'){currWidth=$('#ft-'+el.id+itemId).width();$('#ft-'+el.id+itemId).css({width:0,opacity:0,'background-image':'url("./img_el.id_imgInc_el.id.html")'});$('#ft-'+el.id+itemId).animate({width:currWidth,opacity:1},1000);}else{$('#ft-'+el.id+itemId).css({height:0,opacity:0,'background-image':'url("./img_el.id_imgInc_el.id.html")'});$('#ft-'+el.id+itemId).animate({height:opts[el.id].height,opacity:1},1000);}
